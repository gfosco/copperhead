...
public class TerminalTextController : MonoBehaviour {
	...
	MUDGameMasterScript gameMasterScript;

	// Use this for initialization
	void Start () {
		buffer = new ArrayList();
		textControl = GetComponent<GUIText>();
		gameMaster = GameObject.Find ("MUDGameMaster");
		gameMasterScript = gameMaster.GetComponent<MUDGameMasterScript>(); 
	}

	public void typeText(string text) {
		if (gameMasterScript.secureInput()) {
			text = "*";
		}
	...
	public void backspace() {
		var text = buffer[buffer.Count - 1].ToString();
		buffer[buffer.Count - 1] = text.Substring(0, text.Length - 1);
	...
	void Update () {
		...
		textControl.text = output;
		if (buffer != null && buffer.Count > 100) {
			buffer.RemoveAt (0);
		}
	}
	...
	void handleInput() {
		...
		if (Input.anyKeyDown) {
			if (Input.GetKeyDown (KeyCode.Return) || Input.GetKeyDown (KeyCode.KeypadEnter)) {
				waitingForEntry = false;
				addLines ("");
				gameMasterScript.handleCommand(entry);
			} else if (Input.GetKeyDown (KeyCode.Backspace)) {
				if (entry.Length > 0) {
					backspace();
				}
			} else if (Input.inputString.Length > 0) {
				entry += Input.inputString;
				typeText (Input.inputString);
			}
		}
	}
	...
	// function taken from wiki.unity3d.com/index.php/StringUtil
	...
	string lineWrap( string _str, int _charsPerLine, bool _forceWrap )
	{
