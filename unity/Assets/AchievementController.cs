...
public class AchievementController : MonoBehaviour {
	...
	void Start () {
		queue = new ArrayList();	
		achievementText = GameObject.Find("achievementText").GetComponent<GUIText>();
		hide ();	
	}
	...
	void Update () {
		if (queue.Count > 0 && showing == false) {
			string atext = queue[0].ToString();
			achievementText.text = lineWrap(atext, 38, true);
			queue.RemoveAt(0);
			frameCounter = frameStart;
			show ();
		} else if (showing == true && frameCounter > 0) {
			frameCounter -= 1;
			if (frameCounter == 0) {
				hide ();
			}
